...
class Screen:
    def __init__(self):
        # set up page
        self.root7 = tk.Tk()
        self.root7.title("HAPPY RICHIES")
        self.root7.geometry("1500x1000")

        # push button
        self.daily_button = tk.Button(self.root7, text="Daily", padx=150, pady=10, borderwidth=30, command=self.daily)
        self.daily_button.place(x=200, y=50)
        self.monthly_button = tk.Button(self.root7, text="Monthly", padx=150, pady=10, borderwidth=30,
        ...
        # year push button
        self.year_2021 = tk.Button(self.root7, text="2021", padx=20, pady=10, borderwidth=30, command=self.overall)
        self.year_2021.place(x=200, y=100)
        self.year_2020 = tk.Button(self.root7, text="2020", padx=20, pady=10, borderwidth=30, command=self.years)
        self.year_2020.place(x=290, y=100)
        self.year_2019 = tk.Button(self.root7, text="2019", padx=20, pady=10, borderwidth=30, command=self.years)
        self.year_2019.place(x=380, y=100)
        self.year_2018 = tk.Button(self.root7, text="2018", padx=20, pady=10, borderwidth=30, command=self.years)
        self.year_2018.place(x=470, y=100)
        self.year_2017 = tk.Button(self.root7, text="2017", padx=20, pady=10, borderwidth=30, command=self.years)
        self.year_2017.place(x=560, y=100)
        self.year_2016 = tk.Button(self.root7, text="2016", padx=20, pady=10, borderwidth=30, command=self.years)
        self.year_2016.place(x=650, y=100)
        self.year_2015 = tk.Button(self.root7, text="2015", padx=20, pady=10, borderwidth=30, command=self.years)
        self.year_2015.place(x=740, y=100)
        self.year_2014 = tk.Button(self.root7, text="2014", padx=20, pady=10, borderwidth=30, command=self.years)
        self.year_2014.place(x=830, y=100)
        self.year_2013 = tk.Button(self.root7, text="2013", padx=20, pady=10, borderwidth=30, command=self.years)
        self.year_2013.place(x=920, y=100)
        self.year_2012 = tk.Button(self.root7, text="2012", padx=20, pady=10, borderwidth=30, command=self.years)
        self.year_2012.place(x=1010, y=100)
        self.year_2011 = tk.Button(self.root7, text="2011", padx=20, pady=10, borderwidth=30, command=self.years)
        self.year_2011.place(x=1100, y=100)
        self.year_2010 = tk.Button(self.root7, text="2010", padx=20, pady=10, borderwidth=30, command=self.years)
        self.year_2010.place(x=1190, y=100)

        # input saving
        self.data = us.load_data("./user_data/balance.json")
        self.save = tk.Label(self.root7, text=self.data["4/21"])
        self.save.place(x=1300, y=480)
        self.save_save = tk.Label(self.root7, text="Saving:")
        self.save_save.place(x=1200, y=480)

        # input target save
        self.new_data = us.load_data("./user_data/cal.json")
        self.target = tk.Label(self.root7, text=self.new_data["monthly"])
        self.target.place(x=1300, y=500)
        self.target_save = tk.Label(self.root7, text="Target Saving:")
        self.target_save.place(x=1200, y=500)

        # input Differences
        self.minus = int(self.data["4/21"]) - int(self.new_data["monthly"])
        self.minus_show = tk.Label(self.root7, text=self.minus)
        self.minus_show.place(x=1300, y=530)
        self.dif_save = tk.Label(self.root7, text="Differences:")
        self.dif_save.place(x=1200, y=530)

        # show congrats
        if self.data["4/21"] > self.new_data["monthly"]:
            label = tk.Label(self.root7, text=" ☻ Congratulation!" + "\n" + "GOOD JOB for this month!", bg="green",
                             padx=30,
                             pady=20, width=14, height=10)
            label.place(x=1200, y=560)
        else:
            label = tk.Label(self.root7, text=" ☹ Please" + "\n" + "TRY AGAIN NEXT MONTH", bg="red", padx=30, pady=20,
                             width=14, height=10)
            label.place(x=1200, y=560)

        # table
        self.table = ["Date", "Transaction", "Income", "Expense", "Balance"]
        self.op_table = ttk.Treeview(self.root7, column=self.table, show="headings", height=15)
        for i in self.table:
        ...
        self.bar = FigureCanvasTkAgg(self.figure1, self.root7)
