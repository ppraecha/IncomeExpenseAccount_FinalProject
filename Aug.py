...
class Screen:
    def __init__(self):
        ...
        self.save = tk.Label(self.root11, text=self.data["8/21"])
        self.save.place(x=1300, y=480)
        self.save_save = tk.Label(self.root11, text="Saving:")
        ...
        # input Differences
        self.minus = int(self.data["8/21"]) - int(self.new_data["monthly"])
        self.minus_show = tk.Label(self.root11, text=self.minus)
        self.minus_show.place(x=1300, y=530)
        self.dif_save = tk.Label(self.root11, text="Differences:")
        self.dif_save.place(x=1200, y=530)

        # show congrats
        if self.data["8/21"] > self.new_data["monthly"]:
            label = tk.Label(self.root11, text=" ☻ Congratulation!" + "\n" + "GOOD JOB for this month!", bg="green",
                             padx=30,
                             pady=20, width=14, height=10)
            label.place(x=1200, y=560)
        else:
            label = tk.Label(self.root11, text=" ☹ Please" + "\n" + "TRY AGAIN NEXT MONTH", bg="red", padx=30, pady=20,
                             width=14, height=10)
            label.place(x=1200, y=560)
        ...
        self.bar = FigureCanvasTkAgg(self.figure1, self.root11)
